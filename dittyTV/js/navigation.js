window.addEventListener('load', function () {
    ...
    $('#menuList').on('sn:focused', function (e) {
        ...
        $("#menuList > li").removeClass("menu-color");
        $("ul#menuList li:nth-child(" + (FOCUSED_MENU_INDEX + 1) + ")").focus().addClass("menu-color");
    });
    ...
    $('#homeList').on('sn:focused', function (e) {
        ...
        FIRST_PAGE_FOCUSED_INDEX = $('#' + e.target.id).closest('li').index();
    ...
    $('#catList').on('sn:focused', function (e) {
        ...
        SECOND_PAGE_FOCUSED_INDEX = $('#' + e.target.id).closest('li').index();
        $(".menu_container").removeClass("active");
        $(".menu_container").css('visibility', 'hidden');
    ...
    $('#catList, #subcatList, #homeList').on('sn:enter-down', function (e) {
        ...
        index = $('#' + e.target.id).closest('li').index();
        var onclick = false;
        ...
        if (onclick) $("#" + e.currentTarget.id + " li#" + e.target.id).click();
    });

    // When sub category list focused
    $('#subcatList').on('sn:focused', function (e) {
        console.log("subcat list focused ...");
        THIRD_PAGE_FOCUSED_INDEX = $('#' + e.target.id).closest('li').index();
        ...
        // for pagination
        if (APP_DATA_ARRAY[SELECTED_MENU_INDEX]['menu_type'] == 'S' || APP_DATA_ARRAY[SELECTED_MENU_INDEX].categories[FIRST_PAGE_SELECTED_INDEX]['cat_type'] != 'P') {
            if ($("ul#subcatList li:nth-last-child(-n+12)").is(":focus") && LOAD_NEXT_PAGE) {
        ...
        switch (evt.keyCode) {
            ...
            case 461: // Return/Back key
                ...
                if ($(".menu_container").hasClass("active") && !$(".splash-screen").is(':visible')) {
                    ...
                    $("ul#catList").html("").removeAttr("style");
            ...
            case 37: // LEFT arrow
                ...
                else if ($('#video_container').is(':visible') && !AD_PLAY && catType != "L") {
                    console.log("Rewind Video");
                    rewind_video();
