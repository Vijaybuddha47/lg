...
function _onClickEvent(e) {
    ...
    if ($(".modal_container").hasClass("active")) {
        ...
        FIRST_PAGE_SELECTED_INDEX = $('#' + e.target.id).closest('li').index();
        ...
        if (catType == 'S') {
            ...
            var url = new URL(APP_DATA_ARRAY[SELECTED_MENU_INDEX]['categories'][FIRST_PAGE_SELECTED_INDEX]['stream_url']);
            var wurlURL = url.searchParams.get("ads.wurl_channel");
            console.log(APP_DATA_ARRAY[SELECTED_MENU_INDEX]['categories'][FIRST_PAGE_SELECTED_INDEX]['stream_url'], wurlURL);

            if (wurlURL) AD_PLAY = false;
            else AD_PLAY = true;
        ...
        SECOND_PAGE_SELECTED_INDEX = $('#' + e.target.id).closest('li').index();
        ...
        if (menuType == 'L') {
            ...
            else THIRD_PAGE_SELECTED_INDEX = $('#' + e.target.id).closest('li').index();
            ...
            if (catType == 'S') {
                ...
                PLAY_AUDIO = false;
                THIRD_PAGE_SELECTED_INDEX = $('#' + e.target.id).closest('li').index();
                var obj = CHANNEL_DATA_ARRAY['channels'][THIRD_PAGE_SELECTED_INDEX];
                $("#subCatDesc").text(obj['description']);

                if (SELECTED_AUDIO == "" || SELECTED_AUDIO != "li#" + e.currentTarget.id) {
                    if (SELECTED_AUDIO) closeAudio();

                    SELECTED_AUDIO = "li#" + e.currentTarget.id;
                    load_audio();
                }
            }
