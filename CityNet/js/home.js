            ...
            if (timestamp == TIME_STAMP) {
                if (json.package_info.live_disabled == "1" && MENU_INDEX == 0) {
                    ...
                    if (json.package_info.live_disabled != "1" && json.package_info.vod_disabled == "1") {
                        $(".menu_list_container").addClass("sort_menu_container");
                        if (SELECTED_MENU_INDEX == 0) MENU_INDEX = 0;
                        $("#menu1").hide();
                        $("#menu2").hide();
                        if (MENU_INDEX == 0) set_home_menu_data(json);
                    } else if (json.package_info.vod_disabled != "1" && json.package_info.live_disabled == "1") {
                        if (FOCUSED_MENU_INDEX !== SELECTED_MENU_INDEX) {
                            SELECTED_MENU_INDEX = MENU_INDEX = 2;
                            $(".menu_list_container").removeClass("sort_menu_container");
                            $("#menu1").removeClass("selected");
                            $("#menu2").addClass("selected");
                        }
                        set_movies_and_shows_menu_data(json);
                    } else if (json.package_info.vod_disabled == "0" && json.package_info.live_disabled == "0") {
                        $(".menu_list_container").removeClass("sort_menu_container");
                        $("#menu0").show();
                        $("#menu1").show();
                        $("#menu2").show();
                        if (SELECTED_MENU_INDEX == 1 || SELECTED_MENU_INDEX == 2) set_movies_and_shows_menu_data(json);
                        else if (SELECTED_MENU_INDEX == 0) set_home_menu_data(json);
        ...
        if (json.result == 200) {
            if (json.channels.length > 0) {
                ...
                for (i = 0; i < json.channels.length; i++) {
                    if (json.channels[i]['has_access'] && json.package_info.live_disabled == "0") {
                        ...
                        if (j == 0) downFocus = ' data-sn-down="null"';
...
function set_logout_menu_data() {
    ...
    manage_spatial_navigation(containerClass);
    console.log("7777777...", SELECTED_MENU_INDEX);
}
...
function selected_menu_list() {
    $(".menu_container").find("li").each(function (i) {
        // if (!$("#menu0").is(':visible')) SELECTED_MENU_INDEX = 1;
