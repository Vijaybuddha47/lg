...
function keyboardVisibilityChange(event) {
    ...
    if (visibility) {
        $(".login_screen").addClass("cursor_disabled");
        console.log("Virtual keyboard appeared");
    }
    else {
        $(".login_screen").removeClass("cursor_disabled");
...
function _onClickEvent(e) {
    ...
    if ($(".modal_container").hasClass("active")) {
        ...
        if ($('#noButton').is(":focus") && name == "EXIT") {
            console.log('hide popup');
            hide_show_modal(false, 'EXIT');

            // For Exit popup yes button
        } else if ($('#noButton').is(":focus") && name == "LOGOUT") {
            console.log('hide popup');
            hide_show_modal(false, 'LOGOUT');
            SN.focus("#loginButton");

            // For Exit popup yes button
        } else if ($("#yesButton").is(":focus") && name == "EXIT") {
            console.log('exit app');
            window.close();

            // For Retry popup retry button
        } else if ($("#yesButton").is(":focus") && name == "LOGOUT") {
            console.log('exit app');
        ...
        if ($(".menu_container").hasClass("active")) {
            ...
            if (SELECTED_MENU_INDEX == 3) {
                SELECTED_MENU_INDEX = MENU_INDEX = 3;
            ...
            if ($('.channel_list_container').hasClass('toggle_channel_list') && $("#" + elementId).parent().attr("id") == "live_channel_list") {
                ...
                if (elementId == "customHome") {
                    ...
                    setTimeout(function () {
                        $(".custom_control_container").hide();
                        $(".mejs__time-total").addClass("mejs__offscreen");
                        SN.focus("videoSection");
                    }, 100);
                }
            ...
            else if ($('#video_container').is(':visible') && !$("#" + elementId).parent().attr("id") == "customControls") {
                if (VIDEO_PLAYER.paused) {
                    $("#customPause").find("img").attr("src", "images/pause_icon.png");
                    VIDEO_PLAYER.play();
                }
        ...
        if (PAGE == "home") {
            ...
            if (!KEYBOARD) {
                ...
                if ($("#video_container").hasClass("active") && SELECTED_MENU_INDEX == 0) {
                    if (elementId == "mep_5") SN.remove("#mep_5"), SN.focus("videoSection");
                    else if (elementId == "mep_0") SN.remove("#mep_0"), SN.focus("videoSection");
                    else if (elementId == "mep_4") SN.remove("#mep_4"), SN.focus("videoSection");
                    else if (elementId == "videoPlayer_native_hls") SN.remove("#videoPlayer_native_hls"), SN.focus("videoSection");
                    $(".mejs__time-total").addClass("mejs__offscreen");
                }
