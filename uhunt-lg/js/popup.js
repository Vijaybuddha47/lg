...
function hide_show_modal(action, name, message)
{
	...
	if (action == true && !modalName) {
		...
		} else if ($(".youtube_video_container").hasClass("active")) {
			TAB_INDEX = 7;
		}
		
		// Remove active class from all container and add to modal box
		$(".menu_container, .cat_container, .search_container, .video_details_container, .video_container, .youtube_video_container").removeClass("active");
