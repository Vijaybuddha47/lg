...
function set_channels_and_video_category(timestamp, menuId, page)
{
	if ((menuId == "CC" || menuId == "VC") && timestamp == TIME_STAMP) {
		...
		switch (MENU_ARRAY[SELECTED_MENU_INDEX]['id']) {
			case "VC":  feedUrl = VIDEO_FIRST_PAGE_URL;
						break;
			case "CC":  feedUrl = CHANNEL_FIRST_PAGE_URL; 
						break;
		}
...
function set_search_screen()
{
	if (MENU_ID == "SE") {
		...
		str +=		'<input type="text" class="search_input focusable" id="searchInputText" tabindex="1" data-sn-right="null" data-sn-up="null" data-sn-down="null">';
		str +=	'</div>';
					...
					if (results != undefined && results != "") {
						...
						if (totalItems > 0) {
							if (THIRD_PAGE_COUNTER == 1) {
								...
								str +=			'<p>'+ limit_words(channelObj['description'], 550) +'</p>';
								str +=		'</div>';
								str += '</div>';
...
{
	switch (focusDepth) {
		case 1: pageCounter = FIRST_PAGE_COUNTER;
				break;

		case 2: pageCounter = SECOND_PAGE_COUNTER;
				break;

		case 3: pageCounter = THIRD_PAGE_COUNTER;
				break;
	}
	...
	if (pageCounter < page && (menuId == MENU_ID)) {
		...
		switch (focusDepth) {
			case 1: FIRST_PAGE_ITEM_INDEX = 0;
					FIRST_PAGE_ITEM_COUNTER = 0;
					FIRST_PAGE_COUNTER = 1;
					break;

			case 2: SECOND_PAGE_ITEM_INDEX = 0;
					SECOND_PAGE_ITEM_COUNTER = 0;
					SECOND_PAGE_COUNTER = 1;
					break;

			case 3: THIRD_PAGE_ITEM_INDEX = 0;
...
function set_grid_li(data, totalItemsFromAPI, i, tabindex, focusNamePrefix, menuId, showDuration)
{
	...
	switch (tabindex) {
		case 1: itemCounter = FIRST_PAGE_ITEM_COUNTER;
				rowItemCounter = FIRST_ROW_ITEM_COUNTER;
				break;

		case 2: itemCounter = SECOND_PAGE_ITEM_COUNTER;
				rowItemCounter = SECOND_ROW_ITEM_COUNTER;
				break;

		case 3: itemCounter = THIRD_PAGE_ITEM_COUNTER;
	...
	if (!ignoreLi) {
		switch (tabindex) {
			case 1: if (menuId == "AL" || menuId == "CAS") {
		...
		switch (tabindex) {
			case 1: if (FIRST_ROW_ITEM_COUNTER == 4) FIRST_ROW_ITEM_COUNTER = 0;
					FIRST_ROW_ITEM_COUNTER++;
					FIRST_PAGE_ITEM_COUNTER++;
					break;

			case 2: if (SECOND_ROW_ITEM_COUNTER == 4) SECOND_ROW_ITEM_COUNTER = 0;
					SECOND_ROW_ITEM_COUNTER++;
					SECOND_PAGE_ITEM_COUNTER++;
					break;

			case 3: if (THIRD_ROW_ITEM_COUNTER == 4) THIRD_ROW_ITEM_COUNTER = 0;
...
function call_next_page(totalItemCount, focusDepth)
{
	...
	switch(focusDepth) {
		case 1: counter = FIRST_PAGE_COUNTER; break;
		case 2: counter = SECOND_PAGE_COUNTER; break;
		case 3: counter = THIRD_PAGE_COUNTER; break;
	}
	...
	if (counter <= page) {
		...
		switch (focusDepth) {
			case 1: if (MENU_ID == "AL" || MENU_ID == "CAS") {
			...
			case 2: set_categories_video_menu_second_page(MENU_ID, TIME_STAMP, page);
					break;
			
			case 3: if (MENU_ID == "CAS") {
						if ($("ul#featuredRow").hasClass("active"))
...
function set_video_details_screen()
{
	...
	if (obj.description != undefined && obj.description != "") description = obj.description;
...
function playVideo()
{
	...
	if ((VOD_URL).indexOf(".m3u") > -1) type = type + "hls";
	else type = type + "mp4";
...
{
	$(".main-container").hide();
	if (VOD_URL.indexOf('youtube.com') !== -1) {
		$(".youtube_video_container").show();
		$(".video-inner").show();
		$(".video-loader").show();
	} else $("#video_container").show();
}
...
function forward_video()
{
	if ($(".youtube_video_container").hasClass("active")) {
		var aa = Youtube_Player.getCurrentTime();
		console.log("FF button");
		Youtube_Player.seekTo(aa + MEDIA_FORWARD_INTERVAL, true);
...
function rewind_video()
{
	if ($(".youtube_video_container").hasClass("active")) {
		...
		if (aa > 5) {
			Youtube_Player.seekTo(aa - MEDIA_REWIND_INTERVAL, true);
		}
	} else if ($(".video_container").hasClass("active")) {
