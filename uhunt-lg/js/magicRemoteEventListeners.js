...
function _onClickEvent(e)
{
	...
	if (PAGE == "home") {
				...
				if (VOD_URL.indexOf('youtube.com') !== -1) {
					if (YOUTUBE_PLAYER_STATE == 2) {
						if (navigator.onLine) {
							...
							Youtube_Player.loadVideoById(getYouTubeID(VOD_URL));
							Youtube_Player.playVideo();
						}
					} else {
						Youtube_Player.stopVideo();
		...
		// For menu is clicked
			...
			switch (true) {
				case MENU_ID == "AL":	MENU_LOAD_TIMER = setTimeout(function(){
																				$(".featuredRowContainer, .allVideosGridContainer").show();
																				set_all_videos_and_channels_shows_screen(MENU_ID, TIME_STAMP); 
																				}, time);
										break;

				case MENU_ID == "CAS":	MENU_LOAD_TIMER = setTimeout(function(){
																				$(".featuredRowContainer, .allVideosGridContainer").show();
																				set_all_videos_and_channels_shows_screen(MENU_ID, TIME_STAMP);
																				}, time);
										break;

				case MENU_ID == "VC" || MENU_ID == "CC":	MENU_LOAD_TIMER = setTimeout(function(){
																									$(".allVideosGridContainer, .loader").show();
																									set_channels_and_video_category(TIME_STAMP, MENU_ID, 1);  
																									}, time);
															break;
				
				case MENU_ID == "SU": MENU_LOAD_TIMER = setTimeout(function(){set_support_screen();}, time); break;

				case MENU_ID == "SO": window.location.href = "login.html";

				case MENU_ID == "SE": MENU_LOAD_TIMER = setTimeout(function(){set_search_screen();}, time); break;
		...
		// move to categories menu third page
			...
			if (elementId.search("videoDescriptionImg") > -1) {
				if ($(".video_description_play_icon").is(":visible")) {
					...
					if (VOD_URL.indexOf('youtube.com') !== -1) {
						$(".youtube_video_container").addClass('active');
